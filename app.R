...
ui <- navbarPage('Project NBA',
          
        #First Page Starts Here    
        tabPanel('Seasonal Shooting Stats',
                 ...
                 sidebarLayout(
                   sidebarPanel(
                     ...
                     selectizeInput("desired_players", paste("Desired Players:"), choices = NULL,
                                    multiple = TRUE),
        ...
        tabPanel("Teams' Off-Def Rating Change",
                 ...
                 sidebarLayout(
                   sidebarPanel(
                     ...
                     selectizeInput("selected_teams", paste("Desired Teams:"), choices = NULL,
...
#   })
  ...
  #Main Plot for Page 1
    output$distPlot <- renderPlotly({
      ...
      add_trace(data = player_info[player_info$season == input$selected_season,], 
                  x = ~distance,
                  y = ~sht_percentage,
                  type = "box",
                  boxpoints = "all", 
      ...
      for(player in desired_players){
        players_plot <- players_plot %>% add_trace(data = player_info[(player_info$player == player) & (player_info$season == input$selected_season),],
                           type = 'box',
                           boxpoints = 'all',
                           jitter = 0,
                           pointpos = inside,
  ...
  output$OffDef <- renderPlotly({
    ...
    ggplotly(
      ...
      geom_point(data = team_summ[team_summ$season %in% input$selected_seasons[1] & 
                                  !team_summ$team %in% selected_teams,],
                 aes(x = o_rtg,
                     y = -d_rtg,
                     group = team,
                     text = paste(season, "Season Ratings for", team,
        ...
        geom_point(data = team_summ[team_summ$season %in% input$selected_seasons[2] & 
                                      !team_summ$team %in% selected_teams,],
                   aes(x = o_rtg,
                       y = -d_rtg,
                       group = team,
                   ...
                   size = 3,
                   alpha = 0.1)+
      geom_line(aes(x = o_rtg,
                    y = -d_rtg,
                    group = team),
                alpha = 0.1)+
        labs(x = "Offensive Rating (-> higher is better)",
      ...
      #Selected Teams
      geom_line(data = team_summ[team_summ$season %in% input$selected_seasons & team_summ$team %in% selected_teams,],
                aes(x = o_rtg,
                    y = -d_rtg,
                   color = team))+
      geom_point(data = team_summ[team_summ$season %in% input$selected_seasons[2] & 
